...
func (p *listInterfacesCmd) Execute(ctx context.Context, f *flag.FlagSet, args ...interface{}) subcommands.ExitStatus {
	...
	for r := range resp {
		...
		if !p.namesOnly {
			fmt.Fprintf(state.Out[r.Index], "%-3s %-16s %-20s %6s  %s\n", "idx", "name", "hwaddr", "MTU", "flags")
		}
		for _, iface := range r.Resp.Interfaces {
			...
			if p.namesOnly {
				...
				s = fmt.Sprintf("%-3d %-16s %-20s %6d  %s", iface.Index, iface.Name, net.HardwareAddr(iface.Hwaddr).String(), iface.Mtu, net.Flags(iface.Flags).String())
			}
